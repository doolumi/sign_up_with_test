...
void main() {
  group('E-mail Validation Check', () {
    test('@수 2개 이상: ERROR', () {
      Either<List<ValidateFailure>, String> result =
  ...
  group('Password 조건 1.영문, 숫자, 특수문자 2가지 이상 8~20자', () {
    test('8자 미만 : ERROR', () {
      Either<List<ValidateFailure>, String> result =
          PasswordValidationUseCase().call('123asd');
      expect(result.isLeft(), true);
    });
    test('8자 & 혼합 조건 충족: PASS', () {
      Either<List<ValidateFailure>, String> result =
          PasswordValidationUseCase().call('1234asdf');
      expect(result.isRight(), true);
    });
    test('20자 초과 : ERROR', () {
      Either<List<ValidateFailure>, String> result = PasswordValidationUseCase()
    ...
    test('20자 & 혼합 조건 충족 : PASS', () {
      Either<List<ValidateFailure>, String> result =
          PasswordValidationUseCase().call('1234qwer123123qwe12q');
      expect(result.isRight(), true);
    });

    test('혼합 조건 미충족 + 글자수 조건 충족 : ERROR', () {
      Either<List<ValidateFailure>, String> result =
          PasswordValidationUseCase().call('11112222');
      expect(result.isLeft(), true);
    });
  });
  ...
  group('Password 조건 2. 반복 되거나 동일한 숫자/문자', () {
    test('동일 숫자 : ERROR', () {
      Either<List<ValidateFailure>, String> result =
          PasswordValidationUseCase().call('333');
      expect(result.isLeft(), true);
    });
    test('동일 문자 : ERROR', () {
      Either<List<ValidateFailure>, String> result =
          PasswordValidationUseCase().call('qqq');
      expect(result.isLeft(), true);
    });
    test('연속 숫자 : ERROR', () {
      Either<List<ValidateFailure>, String> result =
          PasswordValidationUseCase().call('123');
      expect(result.isLeft(), true);
    });
    test('떨어진 숫자 : Pass', () {
      Either<List<ValidateFailure>, String> result =
          PasswordValidationUseCase().call('135');
      expect(result.isRight(), true);
    });
    test('연속 문자 : ERROR', () {
      Either<List<ValidateFailure>, String> result =
          PasswordValidationUseCase().call('abc');
      expect(result.isLeft(), true);
    });
    test('떨어진 문자 : ERROR', () {
      Either<List<ValidateFailure>, String> result =
          PasswordValidationUseCase().call('ace');
      expect(result.isRight(), true);
    });
  });
